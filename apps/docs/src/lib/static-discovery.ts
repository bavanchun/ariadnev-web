...
export async function exportStaticDiscovery(catalog: DocsContentCatalog, contentRoot: string, outRoot: string): Promise<readonly string[]> {
  ...
  const concise: string[] = ["# vcskill documentation", "", "Static English and Vietnamese documentation by release.", ""];
  ...
  const llmsFull = `# vcskill documentation — full public Markdown\n\n${fullSections.join("\n---\n\n")}`;
