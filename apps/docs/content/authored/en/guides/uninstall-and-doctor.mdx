...
> **Mutating — pass `--dry-run` first.** `restore` overwrites current files with the archived versions. The pre-write safety backup exists to recover from a mistake, but the ordinary review is `--dry-run`, which lists every file that would change without writing anything.
...
> **Mutating — receipt-scoped only.** `uninstall` deletes only files whose paths and hashes are in the receipt; a file you edited or created after install is left in place. `--dry-run` lists every path it would remove first.
