...
> **The docs version selector is not the installed version.** Choosing an edition in the header only changes which release's documentation you read; it never changes the binary on your machine. To move the binary between releases, run `ariadnev update` (forward) or reinstall the pinned version you want (`ariadnev.com/download/…?version=<semver>`).
