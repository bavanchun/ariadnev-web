...
| State | Transitions to | Produces | Fails closed on |
|---|---|---|---|
| Compile | Policy | Graph digest, lint report | Schema drift, unknown node kind |
| Policy | Execute, or refuse | Authority decision per node | Effect requested with no executor |
| Execute | Checkpoint (each transition) | Event log entries, node result | Executor error, runtime timeout |
| Checkpoint | Execute, or Proof | `checkpoint.json`, `state-current.json` | Envelope corruption, seal mismatch |
| Proof | terminal | Stable JSON envelope | Identity drift, digest mismatch |
