...
|---|---|---|
| Static contracts | Structural, authority, recovery, evidence, capability shape of the kit source. | Any runtime behaviour of a provider executing that graph. |
| Tier 1 | Skill source quality clears the release floor. | That a skill behaves correctly at run time. |
| Tier 2 | Committed scenarios (routing, trajectory, recovery, authority, duplicate-effect) hold in fresh fixtures. | Scenarios not encoded in the fixture suite; general correctness. |
| Tier 3 | An LLM judge's rating of skill copy against a rubric. | Anything about actual execution or a different runtime. |
| Runtime probes | The pinned runtime + model that actually ran did so with the reported result. | A different runtime, a different model, or an unpinned combination. |
