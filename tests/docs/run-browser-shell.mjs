...
async function inspectViewport(browser, width) {
  ...
  if (width === 320 || width === 1440) await page.screenshot({ path: `/tmp/vcskill-docs-${width}.png`, fullPage: true });
  await context.close();
  return layout;
}
...
try {
  ...
  await copySuccess.addInitScript(() => {
    Object.defineProperty(navigator, "clipboard", {
      configurable: true,
      value: { writeText: async (value) => { window.__vcskillCopiedText = value; } },
    });
  });
  ...
  assert.match(await copySuccessPage.evaluate(() => window.__vcskillCopiedText), /#overview$/);
  ...
  process.stdout.write(`${JSON.stringify({ viewports: layouts.length, searchMatrix: true, localeVersionMatrix: true, axeViolations: 0, noScript: true, screenshots: ["/tmp/vcskill-docs-320.png", "/tmp/vcskill-docs-1440.png"] })}\n`);
} finally {
  await browser.close();
