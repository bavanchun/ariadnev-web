...
export function catalogFixture() {
  ...
  return {
    schemaVersion: 1,
    sourceRelease: { mode: "final", version: CURRENT, releaseTag: `vcskill@${CURRENT}`, sourceSha: SHA, generatorSha: SHA, schemaDigest: DIGEST },
    locales: ["en", "vi"],
...
export async function temporaryContent(catalog = catalogFixture()) {
  const root = await mkdtemp(join(tmpdir(), "vcskill-docs-contract-"));
