...
| 3 | [Docs safety, shell, and shared interactions](./phase-03-docs-safety-and-shell.md) | 1, 2 | Complete (slices 2-8 shipped in first pass; deferred items shipped after P5.slice1 CLI split + cap widen 300000→302000: slice 12 heading permalinks, slice 13 chrome-strings full-authority, slice 14 mobile drawer with focus containment + no-JS fallback, slice 15 search UI grouping/loading/localized fallback) |
| 4 | [Authored docs screen experiences](./phase-04-authored-docs-screen-experiences.md) | 1, 2, 3 | Complete (all D01-D11 authored screens carry a dedicated React screen experience — six shared prose components plus D02-D11 topology/matrix compositions across slices A-D; D00 language-chooser + D18 docs-not-found remain app-level by design; PassThrough fallback removed; `pnpm run test:qualification` green, 104 Playwright + 367 native tests) |
| 5 | [Generated reference experiences](./phase-05-generated-reference-experience.md) | 1, 2, 3, 4 | Substantially complete (slices 1-7 shipped: CLI split, skill catalog table + per-category split, retired-route pipeline + alias-collision invariant, D14 provider comparison matrix + per-provider records, D16 workflow legend/topology/adjacency text-first, D17 release-notes edition metadata + links; **all originally-grandfathered ceilings closed (empty array)**; route count 66→444. Deferred: D16 static SVG topology needs safe MDX component registry or build-time SVG asset pipeline — recorded in renderer docstring) |
| 6 | [Marketing surfaces](./phase-06-marketing-surfaces.md) | 1, 2 | Substantially complete (all five M01 macros shipped: hero split, path narrative w/ sticky topology + meaning/evidence/boundary chapters, authority-boundary wrapping workflow+provider as article subregions, evidence-ledger aligned records, terminal-action recompose; M02 broken-graphite recovery shipped; desktop-horizontal hero-path variant. Remaining: header hover/pressed/current polish, one-shot semantic path transition, cross-viewport Playwright walkthroughs → P7) |
| 7 | [Full deterministic verification](./phase-07-full-deterministic-verification.md) | 1–6 | Substantially complete (2026-08-17: Playwright harness end-to-end with 104 tests stable across two clean runs — 94 chromium screens/axe/accessibility-modes/task-outcomes + 5 firefox + 5 webkit journeys. Fixture manifest, per-surface deterministic servers, chromium-only screenshot baselines for M01/M02/D00–D18, WCAG A/AA axe, SC 1.4.10/1.4.12/forced-colors/print modes, 8 plan-critical task outcomes, cross-browser journeys, Lighthouse on-demand runner, runbook at docs/operations/visual-verification-harness.md, and test:visual wired into test:qualification. Honest gaps: no-JS ≤2-interaction journey (already covered by tests/docs/run-browser-shell.mjs; folding pending Node harness retirement), Lighthouse per-commit (~30s/page — CI-scheduled instead), test:qualification shard parallelism (CI infra), automated two-run flake wrapper (manual verification documented).) |
