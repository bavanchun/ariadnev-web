...
| 7 | [Full deterministic verification](./phase-07-full-deterministic-verification.md) | 1–6 | In progress (bootstrap slices 1-2 shipped: typed screen fixture manifest + route-wide structural probes on built docs HTML. Remaining: Playwright production config, screenshot baselines, cross-browser critical journeys, no-JS + ≤2-interaction command journey, fixed VI fixtures, axe + reflow + text-spacing + forced-colors, task-outcome comparisons, Lighthouse + four-group performance gates, sharding + baseline/browser pinning + runbook — full harness is a dedicated multi-day effort) |
