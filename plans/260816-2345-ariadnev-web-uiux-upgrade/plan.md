...
| 5 | [Generated reference experiences](./phase-05-generated-reference-experience.md) | 1, 2, 3, 4 | In progress (slices 1-3 shipped: CLI split into index + per-command detail pages, skill catalog compacted to table + split into per-category detail pages with `-N` chunking for >25-skill categories; **all 10 originally-grandfathered ceilings now closed (grandfathered array empty)**; route count 66→444; D14 provider comparison, D16 workflow SVG, D17 release notes recompose, CLI retired-route output all pending) |
